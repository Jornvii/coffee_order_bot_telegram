...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    admin_link = (
        f"https://t.me/{os.getenv('ADMIN_USERNAME')}" 
    )
    keyboard = [
    ...
    await update.message.reply_text(
        "💬 Need help?\n\n"
        "If you have any questions or issues, click below to contact the admin:",
        reply_markup=reply_markup,
    )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    button_text, callback_data=f"select_{category}_{item_name}"
                )
            ]
        )
...
async def show_customization(
    update: Update, context: ContextTypes.DEFAULT_TYPE, category, item_name
):
    query = update.callback_query
    await query.answer()
...
async def show_order_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton(
                "❌ បោះបង់", callback_data=f"category_{temp_order['category']}"
            ),
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def customize_sugar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sugar_key, sugar_info in SUGAR_OPTIONS.items():
        keyboard.append(
            [
                InlineKeyboardButton(
                    sugar_info["label"], callback_data=f"set_sugar_{sugar_key}"
                )
            ]
        )
    keyboard.append([InlineKeyboardButton("⬅️ ត្រលប់ក្រោយ", callback_data="back_to_order")])
...
async def customize_ice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ice_key, ice_info in ICE_OPTIONS.items():
        keyboard.append(
            [
                InlineKeyboardButton(
                    ice_info["label"], callback_data=f"set_ice_{ice_key}"
                )
            ]
        )
    keyboard.append([InlineKeyboardButton("⬅️ ត្រលប់ក្រោយ", callback_data="back_to_order")])
    ...
    cart.append(
        {
            ...
            "quantity": temp_order["quantity"],
            "total_price": (
                temp_order["base_price"] + SIZE_OPTIONS[temp_order["size"]]["price"]
            )
            * temp_order["quantity"],
        }
    )
...
async def view_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cart:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            "🛒 កន្ត្រករបស់អ្នកទទេ!\n\n" "សូមចាប់ផ្តើមបញ្ចូលទំនិញពីម៉ឺនុយ។", reply_markup=reply_markup
        )
        return
...
async def checkout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📦 តើអ្នកចង់បានវិធីណា?", reply_markup=reply_markup)


# Process order
async def process_order(
    update: Update, context: ContextTypes.DEFAULT_TYPE, delivery_method
):
    query = update.callback_query
    await query.answer()
